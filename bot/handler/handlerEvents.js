	...
	return async function (event, message) {
		...
		*/
		...
		async function onStart() {
			...
			const isDev = (config.devUsers || []).includes(senderID);
			...
			// Détection du préfixe utilisé selon les rôles configurer
			if (body.startsWith(normalPrefix)) {
				currentPrefix = normalPrefix;
				hasPrefix = true;
			} else if (isDev && body.startsWith(devPrefix)) {
				currentPrefix = devPrefix;
				hasPrefix = true;
			} else if (isAdmin && body.startsWith(adminPrefix)) {
				currentPrefix = adminPrefix;
				hasPrefix = true;
			}

			// L'administrateur utilise le préfixe générique par réflexe
			if (isAdmin && hasPrefix && currentPrefix === normalPrefix) {
				const tempArgs = body.slice(normalPrefix.length).trim().split(/ +/);
				const tempCmdName = tempArgs.shift().toLowerCase();
				const hasValidCmd = GoatBot.commands.get(tempCmdName) || GoatBot.commands.get(GoatBot.aliases.get(tempCmdName));
				if (hasValidCmd) {
					await message.reply("⚡ Hiraishin no Jutsu ! Salut mon seigneur, tu n'as pas besoin d'utiliser de préfixe avec moi. Laisse l'Éclair Jaune de Konoha s'en charger directement ! 🍃");
				}
			}

			// Analyse préliminaire pour déterminer si la commande sous-jacente existe
			let checkArgs = hasPrefix ? body.slice(currentPrefix.length).trim().split(/ +/) : body.trim().split(/ +/);
			let checkCmdName = checkArgs.shift().toLowerCase();
			let command = GoatBot.commands.get(checkCmdName) || GoatBot.commands.get(GoatBot.aliases.get(checkCmdName));

			const aliasesData = threadData.data.aliases || {};
			for (const cmdName in aliasesData) {
				if (aliasesData[cmdName].includes(checkCmdName)) {
					command = GoatBot.commands.get(cmdName);
					break;
				}
			}

			// Gestion du mode sans préfixe pour l'administrateur
			if (isAdmin && !hasPrefix) {
				if (!command) {
					return; // Commande inexistante : Silence radio pour éviter les spams collectifs
				}
				currentPrefix = ""; 
			} else if (!hasPrefix) {
				return; // Utilisateur lambda sans préfixe ignoré
			}

			const args = hasPrefix ? body.slice(currentPrefix.length).trim().split(/ +/) : body.trim().split(/ +/);
			...
			try {
				...
				const getText2 = createGetText2(langCode, `${process.cwd()}/languages/cmds/${langCode}.js`, currentPrefix, command);
